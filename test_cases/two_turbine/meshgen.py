...
op = TwoTurbineOptions(level=level, offset=offset, meshgen=True)
dx_refined = None if args.dx_refined is None else op.base_inner_res*0.5**level
locs = op.turbine_geometries
n = len(locs)
...
# Turbine footprints
for i in range(n):
    code += "Point({:d}) = {{xt{:d}-D*0.70711, yt{:d}, 0., dx_inner}};\n".format(point, i, i)
    code += "Point({:d}) = {{xt{:d}, yt{:d}-D*0.70711, 0., dx_inner}};\n".format(point+1, i, i)
    code += "Point({:d}) = {{xt{:d}+D*0.70711, yt{:d}, 0., dx_inner}};\n".format(point+2, i, i)
    code += "Point({:d}) = {{xt{:d}, yt{:d}+D*0.70711, 0., dx_inner}};\n".format(point+3, i, i)
    point += 4
for i in range(n):

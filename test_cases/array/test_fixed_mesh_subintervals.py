...
def test_adjoint_same_mesh(plot=False):
    ...
    **Disclaimer: largely copied from
    ...
    time_partition = TimePartition(
        end_time, 1, dt, fields, timesteps_per_export=dt_per_export, solves_per_timestep=1,
    )
    ...
    pyadjoint.compute_gradient(J, Control(ic['solution_2d']))  # FIXME: gradient w.r.t. mixed function not correct
    ...
    for spaces in ([fs], [fs, fs]):
        ...
        time_partition = TimePartition(end_time, N, dt, fields, timesteps_per_export=dt_per_export)
        ...
        if plot:
            ...
            for i in reversed(range(N)):
                for j, adj_sol in enumerate(reversed(solutions.solution_2d.adjoint[i])):
                    if i < N-1 and j == len(solutions.solution_2d.adjoint[i])-1:
                        continue
        ...
        err = errornorm(_adj_sol, solutions.solution_2d.adjoint[0][0])/norm(_adj_sol)

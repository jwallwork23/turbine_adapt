...
for fp_iteration in range(args.maxiter + 1):
    ...
    mixed_spaces = [
        (P1_vec, P1_ten) for P1_vec, P1_ten in zip(vector_spaces, metric_spaces)
    ]
    ...
    for i, mesh in enumerate(meshes):
        ...
        def export_func():
            ...
            metric_u = hessian_metric(
                recover_hessian(u, mesh=mesh, target_spaces=mixed_spaces[i])
            )
            metric_v = hessian_metric(
                recover_hessian(v, mesh=mesh, target_spaces=mixed_spaces[i])
            )
            metrics[i].interpolate(
                metrics[i] + wq * metric_intersection(metric_u, metric_v)
            )

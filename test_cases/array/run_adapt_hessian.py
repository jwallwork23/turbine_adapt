...
t_epsilon = 1.0e-05
di = os.path.join('outputs', 'hessian')
...
for fp_iteration in range(args.maxiter + 1):
    ...
    vector_spaces = [VectorFunctionSpace(mesh, "CG", 1) for mesh in meshes]
    metric_spaces = [TensorFunctionSpace(mesh, "CG", 1) for mesh in meshes]
    mixed_spaces = [(P1_vec, P1_ten) for P1_vec, P1_ten in zip(vector_spaces, metric_spaces)]
    metrics = None if converged else [Function(P1_ten) for P1_ten in metric_spaces]
    ...
    for i, mesh in enumerate(meshes):
        ...
        def export_func():
            """
            Compute velocity Hessian metric and accumulate
            by L1 normalisation, i.e. time integration.

            Note that the metric components are not normalised
            before combination.
            """
            if converged:
            ...
            if np.isclose(t, start_time):
                ...
                wq *= 0.5
            u, v = split(solver_obj.fields.solution_2d)[0]
            metric_u = hessian_metric(recover_hessian(u, mesh=mesh, target_spaces=mixed_spaces[i]))
            metric_v = hessian_metric(recover_hessian(v, mesh=mesh, target_spaces=mixed_spaces[i]))
            metrics[i].interpolate(metrics[i] + wq*metric_intersection(metric_u, metric_v))

        # Solve forward on current subinterval, extracting velocity metrics

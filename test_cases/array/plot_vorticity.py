...
# Parse arguments
parser = Parser("test_cases/array/plot_vorticity.py")
...
if args.approach == "fixed_mesh":
    ...
    labels = [
        "{:s} elements\n{:s} DoFs".format(elements[level], dofs[level])
        for level in levels
    ]
else:
...
for level, (colour, label) in enumerate(zip(colours, labels)):
    ...
    input_dir = os.path.join(
        os.path.dirname(__file__), "outputs", args.approach, input_dir
    )
    input_file = f"{input_dir}/diagnostic_vorticity.hdf5"
...
plt.savefig(f"{plot_dir}/peak_vorticities.jpg")
...
plt.savefig(f"{plot_dir}/legend_vorticity.jpg", bbox_inches=bbox)
...
for level, (colour, label) in enumerate(zip(colours, labels)):
    ...
    # Plot peak vorticities
    axes.plot(
        time, zeta_max / zeta_max.max(), "-", label=label, color=colour, linewidth=width
    )
...
axes.grid(True)
plt.tight_layout()
plt.savefig(f"{plot_dir}/relative_peak_vorticities.pdf")
